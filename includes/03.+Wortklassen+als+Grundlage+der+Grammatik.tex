...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item lexikalisches Wort vs.\ Wortform
      \Halbzeile
...
\begin{frame}
  ...
  \onslide<+->
  \begin{exe}
    \ex \alert{Es} \alert{wird} \alert{schon wieder} \orongsch{früh dunkel}.
    \pause
    \ex \gruen{Kristine} \orongsch{denkt}, \alert{dass} \alert{es} \orongsch{bald regnen} \alert{wird}.
    \pause
    \ex \gruen{Adrianna} \alert{hat} \orongsch{gestern} \alert{den} \orongsch{Keller inspiziert}.
    \pause
    \ex \gruen{Camilla} \alert{und} \gruen{Emma} \orongsch{sehen} \alert{sich} \alert{die} \orongsch{Fotos} \alert{an}.  \end{exe}
  \Zeile
  \begin{itemize}[<+->]
    \item \orongsch{Bedeutungstragende Wörter}
    \item \alert{Funktionswörter}
    \item \gruen{Eigennamen}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    \item Kombinatorik für \alert{Wortbestandteile} | Morphologie
      \begin{itemize}[<+->]
        \item Wortbestandteile \zB mit \alert{Umlaut} | \textit{r\rot{o}t} -- \textit{r\rot{ö}ter}
        \item oder \gruen{Ablaut} | \textit{h\gruen{e}ben} -- \textit{h\gruen{o}b}
      \end{itemize}
    \item Kombinatorik für \alert{Wörter} | Syntax
      \Zeile
    \item \alert{Zirkuläre oder leere Definitionen?} \onslide<+->\rot{Nein!} 
...
\begin{frame}
  ...
  \begin{minipage}{\textwidth}
    \begin{itemize}[<+->]
      \item Hauptwort, Dingwort, Gegenstandswort
      \item Zeitwort, Tun-Wort
      \item Eigenschaftswort, Beiwort, Wie-Wort
      \item Umstandswort
      \Halbzeile
      \item Dazu die Vermittlungsversuche
        \begin{itemize}[<+->]
          \item \alert{Dingwörter} | kann man anfassen. \onslide<8->{\rot{Nein!}}
            \pause
          \item Die ontologischen Referenten von Substantiven sind physikalische Objekte. \rot{Nein!}
            \Halbzeile
          \item \alert{Wiewort} | Wie ist die Kanzlerin? -- Katatonisch.
          \item \alert{Tun-Wort} | Was macht\slash tut Johanna? -- Laufen.
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item \alert{Substantive} | \textsc{Numerus}, \textsc{Genus}, \rot{\textsc{Person} ?}, \ldots
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item \orongsch{\textit{wandern}} | \textsc{Numerus}, \textsc{Person}, \textsc{Tempus}, \ldots
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{exe}
    \ex
    \begin{xlist}
      \ex Gestern wurde \alert<6->{kein} \rot<3->{guter} \alert<6->{Espresso} serviert.
      \ex Gestern wurde \alert<6->{der} \rot<3->{gute} \alert<6->{Espresso} serviert.
    \end{xlist}
    \pause
    \pause
    \ex
    \begin{xlist}
      \ex Gestern wurden \alert<6->{keine} \rot<5->{guten} \alert<6->{Espressi} serviert.
      \ex Gestern wurden \alert<6->{die} \rot<5->{guten} \alert<6->{Espressi} serviert.
      \ex Gestern wurden \onslide<6->{\alert{$\emptyset$}}\ \rot<5->{gute} \alert<6->{Espressi} serviert.
    \end{xlist}
  \end{exe}
  \Zeile
  \pause
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item Demnach gäbe es \ldots\ \onslide<+->\rot{eine Wortklasse}.
      \Zeile
    \item Genausogut könnte man sagen: Jedes Wort hat \alert{individuelle Eigenschaften}.
    \item Demnach gäbe es \ldots\ \onslide<+->\rot{so viele Wortklassen wie Wörter}.
      \Zeile
