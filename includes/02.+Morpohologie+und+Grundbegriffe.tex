...
\begin{frame}
  ...
  \begin{itemize}[<+->]
   ...
   \Halbzeile 
    ...
    \item oder \textit{-en} bei Verben im Präsens | Plural und \rot{nicht} adressatbezogen
      \Halbzeile
...
\begin{frame}
  ...
  \pause
  \pause
  Der \alert{Stamm} kann somit \rot{nicht} "`der unveränderliche Wortbestandteil"'\\
...
\begin{frame}
  ...
  \begin{tikzpicture}[node distance=1cm, auto]
   ...
   \onslide<4->{\path[<->, trueblue, draw, bend left=30]  (ihr) edge node {\small P2 Pl} (müsst);}
  \end{tikzpicture}
\end{frame}
...
\begin{frame}
  ...
  \onslide<+->
  \onslide<+->
  \alert{Konstituenten} | Bestandteile einer beliebigen Struktur\\
  \Halbzeile
  \onslide<+->
  \alert{Phrasen} | syntaktische Konstituenten mit bestimmten Eigenschaften\\
  \Zeile
  \begin{itemize}[<+->]
    \item Phrasenbestimmung | ähnlich wie \alert{Satzgliedanalyse} aus der Schule
    \item Hilfsmittel: \alert{Tests} auf Phrasenstatus 
    \item aber dennoch immer Unsicherheiten trotz Tests
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \onslide<+->
  \onslide<+->
  Je nach Art des Kopfs -- Eigenname (Name), Substantiv, Pronomen --\\
 ...
 \Halbzeile 
  \begin{center}
    \scalebox{0.8}{\begin{tabular}[h]{lllll}
      \toprule
      \grau{Artikel oder}    & \grau{AP}         & \alert{nominaler}     & \grau{PPs, Adverben}   & \grau{Relativ- und}  \\
