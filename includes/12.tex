...
\Halbzeile
\begin{itemize}[<+->]
	\item		wenn synonyme Lexeme in dieselbe Stelle eines Syntagmas
	\item		und ohne größere Änderung von dessen Bedeutung einsetzbar
	\item		deren „gemeinsamer Nenner“ heißt dann \alert{Archisemem}
\end{itemize}
\onslide<+->
\Zeile
\begin{exe}
	\ex{\label{ex:wortfelder-001}		\textsc{Synonymisches Wortfeld} \alert{\textit{anstrengende Tätigkeit}} (Ausschnitt)\\
		Den Garten umzugraben war ein(e) ziemliche(r) \gruen{Mühe}~/ \gruen{Mühsal}~/ \gruen{Schufterei}~/ \gruen{Plackerei}~/ \gruen{Schinderei}~/ \gruen{Qual}~/ \gruen{Quälerei}~/ \gruen{Leistung}~/ \gruen{Aufgabe}~/ \gruen{Herausforderung}.}
\end{exe}
\end{frame}
...
\Halbzeile
\begin{itemize}[<+->]
	\item		Hyperonyme und (Ko-)Hyponyme dabei aber nur bedingt austauschbar in Syntagma
	\item		weil häufig mit größerer Bedeutungsänderung verbunden
	\item		oberstes Hyperonym heißt \alert{Archilexem}
\end{itemize}
\onslide<+->
\Zeile
\begin{exe}
	\ex\label{ex:wortfelder-002}		\textsc{Hierarchisches Wortfeld} \alert{\textit{Fahrzeug}} (Ausschnitt)
	\begin{xlist}
		\ex{\label{ex:wortfelder-002a}			Sie werden mit de(m/r) \gruen{Fahrzeug}~/ \gruen{Fahrrad}~/ \gruen{Auto}~/ \gruen{Bus}~/ \gruen{Schiff}~/ \gruen{Fähre}~/ \gruen{Yacht}~/ \gruen{Flugzeug}~/ \gruen{Jet}~/ \gruen{Kutsche}~/ \gruen{Schlitten} weiterreisen.}
\onslide<+->
		\ex{\label{ex:wortfelder-002b}			Sie werden mit de(m/r) \gruen{Fahrzeug}~/ \gruen{Fahrrad}~/ \gruen{Auto}~/ \gruen{Bus}~/ \gruen{Schiff}~/ \gruen{Fähre}~/ \gruen{Yacht}~/ *\orongsch{Flugzeug}~/ *\orongsch{Jet}~/ \gruen{Kutsche}~/ \gruen{Schlitten} weiterfahren.}
\onslide<+->
		\ex{\label{ex:wortfelder-002c}			Sie werden mit de(m/r) *\orongsch{Fahrzeug}~/ *\orongsch{Fahrrad}~/ *\orongsch{Auto}~/ *\orongsch{Bus}~/ *\orongsch{Schiff}~/ *\orongsch{Fähre}~/ *\orongsch{Yacht}~/ \gruen{Flugzeug}~/ \gruen{Jet}~/ *\orongsch{Kutsche}~/ *\orongsch{Schlitten} weiterfliegen.}
	\end{xlist}
\end{exe}
\end{frame}
...
\center
\begin{figure}
\tikzset{level distance=75pt,sibling distance=.5pt}
\begin{tikzpicture}
	\Tree [ .\node (level0-right) {\fbox{\alert{\textit{lachen}}}}; [ .\node (level1-left) {\textit{kichern}}; ] [ .\node {\fbox{\textit{lächeln}}}; [ .\node (level2-left) {\textit{grinsen}}; ] [ .{\textit{feixen}} ]	[ .{\textit{schmunzeln}} ]	[ .\node (level2-right) {\textit{strahlen}}; ] ] [ .\node (level1-right) {\textit{wiehern}}; ] ]
	\foreach \Value/\Text in {0/\fbox{\alert{Hyperonym~1}},1/{{Hypo.~1}~/ \fbox{Hypero.~2}},2/{{Hyponym~2}}}
	{
	\node[anchor=west] 
	at ([xshift=.75cm]{level1-right}|-{level\Value-right}) 
	{\Text};
	}
\end{tikzpicture}
\caption{Wortfeld \textit{lachen} \citep[vgl.][39]{Schlaefer2009}}
\end{figure}
\end{frame}
...
\center
\begin{figure}
%\includegraphics[width=0.5\textwidth]{\GRAPHPATH/WEISGERBER1962-184_Wortfeld-aufhLeb}
\includegraphics[width=0.5\textwidth]{\GRAPHPATH/WEISGERBER1962-184_Wortfeld-aufhLeb}
\caption{Wortfeld \textit{Aufhören des Lebens} \citep[ex][184]{Weisgerber1962}}
\end{figure}
\end{frame}
...
\onslide<+->
formbasierte Gruppierungen selbstständiger Lexeme um ein \alert{Kernlexem}
\Halbzeile
\begin{itemize}[<+->]
	\item		zugeordnete Lexeme heißen \gruen{Familienlexeme}
\end{itemize}
\onslide<+->
\Zeile
...
\Zeile
\begin{exe}
	\ex{\label{ex:wortfamilien-001}		\textsc{Wortfamilie} \textit{fahren} (Ausschnitt)\\
		fahren~\textsubscript{\alert{Kernlexem}} --- befahren, Fahrt, Fuhre, Fähre, Gefährte, Fahrkarte, Fahrrad, Geisterfahrer, Fahrzeug, fahrbereit, fahrig~\textsubscript{\gruen{Familienlexeme}}}
\onslide<+->
	\ex{\label{ex:wortfamilien-002}		\textsc{Wortfamilie} \textit{zaudern} (komplett)\\
		zaudern~\textsubscript{\alert{Kernlexem}} --- (das) Zaudern, Zauderei, Zauderer, Zauderin~\textsubscript{\gruen{Familienlexeme}}}
\end{exe}
\end{frame}
...
\Zeile
\begin{exe}
	\ex\label{ex:wortfamilien-003}		\textsc{Wortfamilie} \textit{trinken} (Ausschnitt)
    \begin{xlist}
		\ex{\label{ex:wortfamilien-003a}		\textsc{Komposition}\\
			trinken~\textsubscript{\alert{Basis}} --- Trinkwasser, Trinkspruch, Zaubertrank, Heißgetränk, Trunkenbold, trinkfest, siegestrunken~\textsubscript{\gruen{Wortbildungsprodukte}}}
		\onslide<+->
		\ex{\label{ex:wortfamilien-003b}		\textsc{Derivation}\\
			trinken~\textsubscript{\alert{Basis}} --- austrinken, betrinken, ertränken, Getränk, Trinker, Tränke, Trunkenheit, trinkbar, trunken~\textsubscript{\gruen{Wortbildungsprodukte}}}
				\onslide<+->
		\ex{\label{ex:wortfamilien-003c}		\textsc{Konversion}\\
			trinken~\textsubscript{\alert{Basis}} --- tränken, Trank, Trunk, (das) Trinken~\textsubscript{\gruen{Wortbildungsprodukte}}}
	\end{xlist}
\end{exe}
\end{frame}
...
\Zeile
\begin{exe}
	\ex\label{ex:wortfamilien-004}		\textsc{Wortfamilie} \textit{Wald} (Ausschnitt)
    \begin{xlist}
		\ex{\label{ex:wortfamilien-004a}		\textsc{Substantive}\\
			Wald~\textsubscript{\alert{Basis}} --- Urwald, Bewaldung, Wäldchen, Waldlosigkeit, Hinterwäldler, Eichenwald, Waldrand~\textsubscript{\gruen{Wortbildungsprodukte}}}
		\onslide<+->
		\ex{\label{ex:wortfamilien-004b}		\textsc{Adjektive}\\
			Wald~\textsubscript{\alert{Basis}} --- waldig, waldlos, hinterwäldlerisch, waldreich~\textsubscript{\gruen{Wortbildungsprodukte}}}
				\onslide<+->
		\ex{\label{ex:wortfamilien-004c}		\textsc{Verben}\\
			Wald~\textsubscript{\alert{Basis}} --- bewalden, entwalden~\textsubscript{\gruen{Wortbildungsprodukte}}}
				\onslide<+->
		\ex{\label{ex:wortfamilien-004d}		\textsc{Adverbien}\\
			Wald~\textsubscript{\alert{Basis}} --- waldein, waldwärts~\textsubscript{\gruen{Wortbildungsprodukte}}}
	\end{xlist}
\end{exe}
\end{frame}
...
\onslide<+->
\Zeile
\gruen{Familienlexeme} stellen Wortbildungsprodukte dar
\Halbzeile
\begin{itemize}[<+->]
...
\center
\begin{figure}
%\includegraphics[width=0.6\textwidth]{\GRAPHPATH/HARM2015-95_Wortfamilie-binden}
\includegraphics[width=0.6\textwidth]{\GRAPHPATH/HARM2015-95_Wortfamilie-binden}
...
\Halbzeile
\begin{itemize}[<+->]
	\item		da Wortbildungsrelation zum Kernlexem durch Sprachwandelprozesse verdunkelt
	\item		oft Zusammenspiel formaler und inhaltlicher Entwicklungen
	\item		Ursprung von „falschen“ Volksetymologien
\end{itemize}
\onslide<+->
\Zeile
\begin{exe}
	\ex{\label{ex:wortfamilien-005}		\textsc{Verdunkelte Familienzugehörigkeiten}\\
		Haft (haben), Zaum, Zucht, zucken, Herzog (ziehen), Witz (wissen), Elend (Land), bitter (beißen), Lager (liegen), Stadt, Stuhl (stehen), Wand, Windel (winden), Kunst (können), schnitzen (schneiden), Welpe (Wolf)}
\onslide<+->
	\ex{\label{ex:wortfamilien-006}		\textsc{Volksetymologien}\\
		einbläuen (*\orongsch{blau}, ahd.~\gruen{bliuwan} ‚schlagen‘), Zierrat (*\orongsch{Zier.art}, \gruen{Zier:at}), mundtot (*\orongsch{blau}, ahd.~\gruen{munt} ‚Schutz‘), Wahnsinn (*\orongsch{Wahn}, mhd.~\gruen{wan} ‚leer, fehlend‘)}
\end{exe}
\end{frame}
