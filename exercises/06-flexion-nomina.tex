...
\begin{enumerate}\Lf
  ...
  \item \textit{er}-Maskulina (Mer) bzw. -Neutra (Ner)\footnotemark[1] % Strauch, Holz
  \item \textit{e}-Feminina (Fe)\footnotemark[1] % Hand
  \item im Plural endungslose Feminina (F$-$)\footnotemark[1] % Tochter
  \item (\textit{e})\textit{n}-Feminina (Fn)\footnotemark[2] % Achse, Schicht
...
\begin{enumerate}
  ...
  \item\doublespacing
    \begin{spread}
      \textbf{Situation}: Max schickt Julius mit dem firmeneigenen Briefboten einen konspirativen Brief über den Vorstand.\\
      \textbf{Sätze}: \uline{Max} weiß, dass \uline{er} in \uline{dem Brief} an \uline{Julius} über \uline{den Vorstand} keine vertraulichen Details über \uline{seine} Beschlussfindung preisgeben darf.
